...
func Test_Desribe(t *testing.T) {
	...
	exporter := NewExporter(metricOptions)
...
func Test_Collect(t *testing.T) {
	...
	expectedQueryResult := client.QueryResult{
		URL:          "https://www.sampleurl.com",
		Status:       client.UP,
		ResponseTime: time.Duration(2 * time.Second),
	}

	exporter.QueryResults = []client.QueryResult{expectedQueryResult}
	...
	result := readGauge(g)
	if expectedQueryResult.URL != result.labels["url"] {
	...
	if expectedQueryResult.URL != hResult.labels["url"] {
